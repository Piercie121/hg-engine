		...
		case SBA_MEGA:
			for (client_no = 0; client_no < client_set_max; client_no++)
			{
				...
				if (flag)
				{
					...
					if (sp->battlemon[client_no].species == SPECIES_AMPHAROS)
					{
						// nest it so that lopunny form 3 handling is not grabbing from the table
						if (sp->battlemon[client_no].item == ITEM_AMPHAROSITE && sp->battlemon[client_no].form_no == 2)
						{
							sp->battlemon[client_no].form_no = 3;
							BattleFormChange(client_no, 3, bw, sp, FALSE); // temp form id 
						}
						else if (sp->battlemon[client_no].item == ITEM_AMPHAROSITE && sp->battlemon[client_no].form_no == 0)
						{
							sp->battlemon[client_no].form_no = 1;
							BattleFormChange(client_no, 1, bw, sp, FALSE); // temp form id
						}
					}
					else
					{
